...
| **Version:** 2.29.6 (guardrail false-positive fixes: context-aware injection guard, URL-encoded angle-bracket guard)
| **Date:** 2026-09-22
...
In decisions mode the prompt digest is sent as the `state` field, and the tier rubric is sent natively as a choice question. The default criteria strings follow TypeSafe's Choice best practices (docs.typesafe.ai/primitives/choice): adjacent tiers are confusable, so each option description separates itself from its neighbors with a `WHAT` / `NOT FOR` / `EXAMPLES` structure. OpenRouter's decisions endpoint accepts strings only (its Zod schema rejects the structured-object form from TypeSafe's own API), so the structure is encoded into the strings:
...
- **`classification.tier_criteria`** (default `{}`) — optional per-tier rubric descriptions sent as the choice criteria; empty uses the built-in L1–L5 rubric summaries (v2.21.0: rewritten in WHAT / NOT FOR / EXAMPLES format per TypeSafe's Choice docs). Custom overrides should keep the same format.
...
|----------|-------|----------|
| Direct override | `injection-ignore-previous` (context-aware guard v2.29.6), `injection-role-assign`, `injection-delimiter` | CRITICAL / HIGH |
...
**False-positive reduction:** Messages dominated by fenced code blocks (≥2 fences, >400 chars) are skipped — educational/discussion contexts. The `injection-ignore-previous` rule uses `ContextAwareInjectionValidator` (v2.29.6) — suppresses findings when the ±80-char window around the match contains educational/defensive context words (prevent, example, attack, detect, security, research, etc.), allowing security-focused conversations that quote injection phrases. The `encoded-unicode` rule uses `EncodedUnicodeValidator` — decodes `\uXXXX` runs and only flags when decoded content contains injection signals (v2.29.4).
...
#### 9.3.14 Obfuscation Scanning (v2.12.0, Base64 removed v2.28.0)

Input-side detection of encoded injection payloads in `rules.py` → `scan_obfuscated_payloads()`, called from `GuardrailEngine.scan_obfuscation()` in the input pipeline (`_guardrail_scan_input()` in `chat.py`).

- **Hex payloads** (`obfuscation-hex`) — hex tokens ≥ 32 chars are decoded; decoded content must contain injection-signal keywords (`_HEX_INJECTION_SIGNALS`: ignore, override, bypass, system, pretend, admin, execute, command, script, eval, etc.) — tokens that decode to benign readable ASCII (file hashes, session IDs, tracking tokens) are skipped (v2.29.5 guard).
- **URL-encoded payloads** (`obfuscation-url-encoded`) — tokens with ≥ 2 `%`-escapes that URL-decode to alphabetic text. Decoded content is checked against `_STRONG_INJECTION_CHARS` (`|;{}\n\r\t\`$\\`, always flag) and weak `<>` (only flag when accompanied by an HTML injection pattern via `_HTML_INJECTION_RE` — `<script`, `<img`, `</system`, `onerror=`, `javascript:`, etc. — or an injection-signal keyword). Benign HTML/JSON-LD markup (`<div>`, `<p>`) from scraped web pages is skipped (v2.29.6 guard).
- **Base64 payloads** (`obfuscation-base64`) — **removed in v2.28.0.** The Shannon-entropy heuristic (≥ 4.5 bits/char) plus decoded-preview prose filters could not reliably separate real injection payloads from legitimate base64 content (JWT segments, data URIs, hashes, encoded tool output) and produced recurring false-positive blocks. Findings severity is now uniformly MEDIUM (the former HIGH special case applied only to base64).
- **Removed config:** `telemetry.guardrails.entropy_threshold` — the entropy heuristic was part of the base64 scan and has no remaining consumer. Stale keys are harmless but should be dropped from `settings.json`.

```mermaid
flowchart TD
    A["Input text"] --> B{"obfuscation_detection<br/>enabled?"}
    B -->|no| Z["pass through"]
    B -->|yes| C["scan_obfuscated_payloads()"]
    C --> HEX["Hex scan<br/>tokens ≥ 32 hex chars<br/>decode → injection-signal guard<br/>_HEX_INJECTION_SIGNALS (v2.29.5)"]
    C --> URL["URL-encode scan<br/>≥ 2 %-escapes<br/>decodes to alphabetic text"]
    C -.->|removed v2.28.0| B64["Base64 + Shannon entropy scan<br/>obfuscation-base64 (HIGH)"]
    B64 -.->|persistent false positives| X["❌ rule deleted"]
    HEX -->|"decoded contains<br/>injection keyword"| F["obfuscation-hex (MEDIUM)<br/>decoded preview + error span"]
    HEX -->|"decoded is benign<br/>readable ASCII"| Z
    URL --> URD{"decoded chars check<br/>(v2.29.6 guard)"}
    URD -->|"strong chars present<br/>|;{}\\n\\r\\t`$\\\"| G["obfuscation-url-encoded (MEDIUM)<br/>decoded preview + error span"]
    URD -->|"<> only: HTML injection<br/>pattern or injection keyword?"| G
    URD -->|"<> only: benign HTML<br/>(div, p, span)"| Z
    URD -->|"no injection chars"| Z
    F --> LOG["router.guardrail.obfuscation findings<br/>input_action: log | block"]
    G --> LOG
```

**Metrics:** `router_guardrail_findings_total{direction="input",rule_id="obfuscation-hex|obfuscation-url-encoded"}`.

**Config:** `telemetry.guardrails.obfuscation_detection` (default: `true`, hot-reloadable).
...
| **M29 — Opt-in Custom Guardrail (v2.24.0)** | `telemetry.guardrails.custom` (disabled by default; single global `enabled` toggle). When enabled, the router evaluates the REQUEST payload only (input path, after the standard injection guardrails, before IP redaction) with a TypeSafe question (`POST /v1/systemone`) that is **fully defined in settings — no code changes needed**: `question_type` ("noul" | "choice" | "score"), `question_id` (routing key in the request/response map), `prompt` (the question instructions, or `prompt_file` on disk), `criteria` (shape follows the type: `{true, false}` for noul, an options map for choice, an ordered levels list for score), and `rejection_message` (client-facing rejection text, `{reason}` placeholder). The answer is mapped to a probability-of-yes — `noul` directly, `probabilities["yes"]` for choice (falling back to the selected option), `score / (levels − 1)` for score — range-checked and thresholded (`yes_threshold`, default 0.5) into a Pydantic `Literal["yes","no"]`; pass → request continues untouched, below threshold → standardized 400 rejection (`code: router_custom_guardrail_rejected`) with the probability logged + counted. Prompt/criteria hot-reloadable (settings + admin reload); `on_error: pass` (fail-open default) / `reject` (fail-closed); non-TypeSafe base_url falls back to any OpenAI-compatible chat endpoint. Metrics: `router_custom_guardrail_evals_total{decision,source}`, `router_custom_guardrail_blocks_total`. Per-agent gating was removed in v2.23.1; output-phase evaluation was removed in v2.24.0 (input only). | 555 tests pass; container rebuilt and live-verified (pass + reject via Noul probability; choice/score types unit- and live-tested). |
| **M30 — Fully Settings-Driven Custom Guardrail (v2.25.0)** | `telemetry.guardrails.custom` question is now defined entirely in settings — no code changes needed to customize the guardrail. New fields: `question_type` ("noul" \| "choice" \| "score"), `question_id` (routing key in the systemone request/response map), `criteria` (settings override; shape follows the type — `{true,false}` for noul, options map for choice, ordered levels for score; built-in Noul criteria remain the default), and `rejection_message` (client-facing rejection text, `{reason}` placeholder; empty = built-in default). `CustomGuardrailSettings.build_question()` constructs the question; the parser maps every answer type to a probability-of-yes (`noul` direct; `probabilities["yes"]` for choice with selected-option fallback; `score / (len(legend) − 1)` for score), range-checked and thresholded as before. `CustomGuardrailSchema` in `app/config/schema.py` synced so `POST /admin/settings/reload` carries the new fields. Defaults preserve v2.24.0 behavior exactly. README adds a live-tested worked example (topic-scope guardrail: on-topic noul 0.96–0.99 pass, off-topic 0.01 → 400, choice P(yes)=1.00 and score 1.99/2 → 0.995 on the same payload, ~0.6 s per decision). | 29 guardrail unit tests pass (18 existing + 11 new in `tests/unit/test_custom_guardrail_settings.py`); container rebuilt as 2.25.0 and live-verified end-to-end. |
| **M31 — Obfuscation Base64 Rule Removal (v2.28.0)** | Removed the `obfuscation-base64` guardrail rule entirely from `app/guardrails/rules.py`: deleted `_scan_base64_payloads`, `_B64_TOKEN_RE`, `calculate_shannon_entropy`, `_try_decode`, `_looks_like_prose`, and the `entropy_threshold` parameter from `scan_obfuscated_payloads()`. Removed the `entropy_threshold` config field from `GuardrailConfig` (scanner.py), the Pydantic settings schema, `chat.py` wiring, and `config/settings.json`. Obfuscation findings are now uniformly MEDIUM severity (the former HIGH special case applied only to base64). Hex and URL-encoding scans are unchanged. Rationale: the Shannon-entropy heuristic (≥ 4.5 bits/char) plus prose filters could not reliably separate real injection payloads from legitimate base64 content (JWT segments, data URIs, hashes, encoded tool output) and caused recurring false-positive blocks. Tests updated: base64 detection test inverted to a "base64 is NOT flagged" regression guard; entropy helper test removed. Spec §9.3.14 added with a mermaid scan-flow diagram. | 584 unit & integration tests pass; container rebuilt as 2.28.0 and live-verified (health 200; base64 payloads no longer flagged, hex/URL scans still detect). |
| **M32 — Guardrail False-Positive Fixes (v2.29.6)** | Two guardrail false-positive fixes: (1) `ContextAwareInjectionValidator` in `app/guardrails/base.py` — registered for the `injection-ignore-previous` rule in `scanner.py::_build_default_registry()` instead of plain `RegexValidator`; after a regex match, examines a ±80-char window for educational/defensive context words (prevent, example, attack, detect, security, research, etc.) and suppresses the finding if any are present. Prevents CRITICAL blocks on security discussions, log-analysis requests quoting block messages, and documentation that references injection phrases. Real injections outside benign context windows still flag CRITICAL. (2) Obfuscation-url-encoded angle-bracket guard in `app/guardrails/rules.py::_scan_url_encoded_payloads()` — split `_HIDDEN_INJECTION_CHARS` into `_STRONG_INJECTION_CHARS` (`\|;{}\n\r\t\`$\\`, always flag) and weak `<>` (only flag when `_HTML_INJECTION_RE` matches — `<script`, `<img`, `</system`, `onerror=`, `javascript:`, etc. — or decoded content contains an `_HEX_INJECTION_SIGNALS` keyword). Prevents MEDIUM blocks on benign HTML/JSON-LD markup (`<div>`, `<p>`, `<span>`) from scraped web pages. Real script/img/semicolon injections still flagged. Spec §9.3 mermaid diagram and prose updated. | 608 unit & integration tests pass (9 new regression tests); container rebuilt as 2.29.6 and live-verified in-container (FP cases return `[]`; real attacks still flagged). Released: git tag, GitHub release, Docker Hub dual-tag. |
