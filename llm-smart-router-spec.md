...
| **Version:** 2.25.0 (custom guardrail: fully settings-driven TypeSafe question, input only)
| **Date:** 2026-09-19
...
- **Hex payloads** (`obfuscation-hex`) — hex tokens ≥ 32 chars are decoded; only tokens whose decoded bytes are ≥ 70 % printable ASCII with letters are flagged (hashes, UUIDs, Docker IDs, git commits decode to binary garbage and are skipped).
- **URL-encoded payloads** (`obfuscation-url-encoded`) — tokens with ≥ 2 `%`-escapes that URL-decode to alphabetic text.
...
flowchart TD
    ...
    C --> HEX["Hex scan<br/>tokens ≥ 32 hex chars<br/>decode → ≥70% printable ASCII"]
    ...
    HEX -->|flag| F["obfuscation-hex (MEDIUM)<br/>decoded preview + error span"]
    URL -->|flag| G["obfuscation-url-encoded (MEDIUM)<br/>decoded preview + error span"]
    HEX -->|clean| Z
    URL -->|clean| Z
