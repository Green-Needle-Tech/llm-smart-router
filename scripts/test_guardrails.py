...
def main():
    ...
    blocked = status == 400 and data.get("error", {}).get("code") == "router_guardrail_blocked"
    results.append(("injection-block", blocked,
                    f"HTTP {status} (block mode: {'BLOCKED' if blocked else 'logged/passed-through'})"))
